...
void CRtspSession::Handle_RtspSETUP()
{
...
#if RTSP_REQUIRE_TCP
    // Decline plain UDP and let the client retry interleaved. Measured on this
    // board: over TCP a 90s session delivered 502 frames with no errors at all,
    // while UDP logged "RTP: missed 14-26 packets" repeatedly - the WiFi link
    // stalls for hundreds of milliseconds at a time, and RTP has no
    // retransmission, so whole seconds of video simply vanish. That is what a
    // viewer sees as a freeze that holds and then drops the session. 461 is the
    // RFC 2326 answer for a transport the server will not do, and clients treat
    // it as "retry with TCP". Build with -DRTSP_REQUIRE_TCP=0 to allow UDP again.
    if (!m_TcpTransport) {
        static char Reject[512];
        snprintf(Reject, sizeof(Reject),
                 "RTSP/1.0 461 Unsupported Transport\r\nCSeq: %u\r\n%s\r\n\r\n",
                 m_CSeq, DateHeader());
        socketsend(m_RtspClient, Reject, strlen(Reject));
        return;
    }
#endif
    ...
    {
        // Report where the RTP actually goes. Upstream hardcodes
        // "destination=127.0.0.1;source=127.0.0.1" here while CStreamer sends to
        // the peer address from socketpeeraddr() - so the packets arrive but a
        // client that believes the header discards them. ffmpeg hangs forever on
        // UDP for exactly this reason (TCP interleaved is unaffected). "source"
        // is optional per RFC 2326 and needs a local-address glue call the other
        // platform headers do not have, so it is simply omitted.
        IPADDRESS otherip;
        IPPORT otherport;
        socketpeeraddr(m_RtspClient, &otherip, &otherport);
        snprintf(Transport,sizeof(Transport),
                 "RTP/AVP;unicast;destination=%u.%u.%u.%u;client_port=%i-%i;server_port=%i-%i",
                 (unsigned)otherip[0], (unsigned)otherip[1],
                 (unsigned)otherip[2], (unsigned)otherip[3],
                 m_ClientRTPPort,
                 m_ClientRTCPPort,
    ...
    snprintf(Response,sizeof(Response),
             ...
             "%s\r\n"
             "Transport: %s\r\n"
             // %u, not %i: the constructor forces bit 31 set, so the signed form
             // emits a negative token that stricter clients reject outright.
             "Session: %u\r\n\r\n",
             m_CSeq,
             DateHeader(),
             Transport,
             (unsigned)m_RtspSessionID);
