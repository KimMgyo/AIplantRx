...
// Increments once per completed panel refresh (last flush of a refresh cycle).
// The camera page uses it to scale a new frame only after the previous one has
// actually reached the panel, instead of redrawing frames that are never shown.
volatile uint32_t g_lvgl_refr_count = 0;
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        if (drv->full_refresh) {
             ...
             * Bounded wait (~4 frames @39Hz) instead of portMAX_DELAY: if the
             * RGB refresh-finish ISR notification is ever missed, we drop/tear a
             * frame rather than deadlock the LVGL task forever (which would also
             * kill net_poll() running as an lv_timer and never recover WiFi). */
            ...
            if (probe_result == FLUSH_PROBE_FULL_COPY) {
                ...
                /* Waiting for the current frame buffer to complete transmission.
                 * Bounded wait (~4 frames @39Hz), not portMAX_DELAY: proceed on a
                 * missed refresh-finish ISR notify rather than deadlocking the LVGL
                 * task (and net_poll() lv_timer) forever. */
                ulTaskNotifyValueClear(NULL, ULONG_MAX);
                ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(100));
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    /* Action after last area refresh */
    if (lv_disp_flush_is_last(drv)) {
        // One completed panel refresh. The camera page gates on this so it scales
        // a new frame only after the previous one has actually reached the panel.
        g_lvgl_refr_count++;
         ...
         * Bounded wait (~4 frames @39Hz), not portMAX_DELAY: proceed on a missed
         * refresh-finish ISR notify rather than deadlocking the LVGL task (and
         * net_poll() lv_timer) forever. */
        ulTaskNotifyValueClear(NULL, ULONG_MAX);
        ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(100));
    }
